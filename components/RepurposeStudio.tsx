...
import { Source, Draft, Platform, DraftStatus, GateResult } from '@/lib/types';
...
function deriveGateResult(gate: Omit<Draft['brandVoiceGate'], 'result'>, proofExists: boolean): GateResult {
  ...
  if (!gate.proofDensity || !proofExists) return 'Needs proof';
...
export function RepurposeStudio({ sources, drafts, setDrafts, selectedSourceId, onNavigate }: RepurposeStudioProps) {
  ...
  const toggleProof = () => {
    if (!activeDraft) return;
    setDrafts(drafts.map(d => d.id === activeDraft.id ? { ...d, proof: { ...d.proof, exists: !d.proof.exists } } : d));
  };
  ...
  return (
    <div className="flex gap-0 h-[calc(100vh-6rem)] animate-in fade-in slide-in-from-bottom-2 duration-500 -mt-8 -mx-8 relative -top-8 h-[calc(100vh)]">
      <div className="w-[320px] bg-[#fafafa] border-r border-ed-border flex flex-col pt-8">
        ...
        <div className="flex-1 overflow-y-auto px-4 space-y-2 pb-8">
            {promotedSources.map(src => (
              <div 
                ...
                className={`p-4 bg-white cursor-pointer transition-colors border-b border-ed-border hover:border-ed-ink group ${activeSourceId === src.id ? 'border-ed-ink' : 'border-transparent'}`}
              >
        ...
        {activeSourceId && (
          <div className="border-t border-ed-border bg-white p-6">
            <h4 className="text-[10px] text-ed-muted uppercase tracking-widest mb-3 border-b border-ed-border pb-2">Target Platform</h4>
              ...
              {platforms.map(p => {
                ...
                return (
                  <button
                    ...
                    className={`text-left px-3 py-2 rounded text-sm flex justify-between items-center transition-colors ${activePlatform === p ? 'bg-ed-ink text-white font-medium' : 'text-ed-ink hover:bg-gray-100'}`}
                  >
                    <span className="text-[13px]">{p}</span>
                    {hasDraft && <span className={`w-2 h-2 rounded-full ${activePlatform === p ? 'bg-white/50' : 'bg-ed-ink'}`} />}
                  </button>
                )
              })}
            </div>
            
            <div className="mt-6 pt-4 border-t border-ed-border">
              <h4 className="text-[10px] text-ed-muted uppercase tracking-widest mb-2">Source Context</h4>
              <p className="text-[11px] text-ed-muted mb-1"><strong className="text-ed-ink font-semibold">Angle:</strong> {activeSource.offerAngle}</p>
              <p className="text-[11px] text-ed-muted"><strong className="text-ed-ink font-semibold">Proof needed:</strong> {activeSource.proofNeeded}</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="flex-1 flex flex-col h-full bg-white">
        <header className="px-12 py-6 border-b border-ed-border flex justify-between items-center bg-white h-[85px] shrink-0">
          <div>
            <h2 className="font-serif text-[1.8rem] font-bold text-ed-ink flex items-center gap-4">
              {activePlatform}
          ...
          {!activeDraft && (
            <button onClick={handleGenerateDraft} disabled={draftLoading} className="px-6 py-2 bg-ed-ink text-white font-semibold text-[0.85rem] border-none rounded-none hover:bg-black transition-colors shrink-0 whitespace-nowrap disabled:bg-ed-border disabled:text-ed-muted disabled:cursor-wait">
        ...
        {activeDraft ? (
          <div className="flex-1 overflow-y-auto flex">
            <div className="w-2/3 p-12 border-r border-ed-border space-y-6">
              ...
              <div>
                <label className="block text-[10px] text-ed-muted uppercase tracking-widest mb-2 border-b border-ed-border pb-1">Main Point (본문)</label>
                <textarea rows={12} value={activeDraft.content.mainPoint} onChange={e => updateDraftField('mainPoint', e.target.value)} className="w-full border-none p-0 text-[14px] focus:outline-none bg-transparent hover:bg-[#fafafa] transition-colors resize-none leading-relaxed text-ed-ink" />
              </div>
              <div>
                <label className="block text-[10px] text-ed-muted uppercase tracking-widest mb-2 border-b border-ed-border pb-1">Call to Action (행동 유도)</label>
                <input value={activeDraft.content.cta} onChange={e => updateDraftField('cta', e.target.value)} className="w-full border-none p-0 text-[14px] focus:outline-none bg-transparent hover:bg-[#fafafa] transition-colors text-ed-ink py-2" />
              </div>
              <div className="p-4 border border-ed-border bg-[#fafafa] mt-8">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <span className="text-[10px] text-ed-muted uppercase tracking-widest">Proof Artifact</span>
                    <span className="text-[13px] font-semibold text-ed-ink">{activeDraft.content.proofArtifactNeeded}</span>
                  </div>
                  <button 
                    onClick={toggleProof}
                    className={`px-4 py-1.5 text-[11px] font-semibold uppercase tracking-widest border transition-colors ${activeDraft.proof.exists ? 'bg-ed-ink text-white border-ed-ink' : 'bg-transparent text-ed-ink border-ed-ink hover:bg-[#eee]'}`}
                  >
                    {activeDraft.proof.exists ? '증명 첨부됨 ✓' : '+ 증명 자료 확인'}
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="w-1/3 bg-[#fafafa] p-8 flex flex-col overflow-y-auto">
              <h3 className="text-[10px] text-ed-muted uppercase tracking-widest mb-4">Brand Voice Gate</h3>
              <p className="text-[13px] text-ed-ink mb-6 leading-relaxed font-serif italic border-b border-ed-border pb-6">&quot;이 글이 흔한 AI의 글처럼 보이지 않도록, 브랜드의 실제 경험과 관점이 담겼는지 체크하세요.&quot;</p>
              ...
              <div className="mb-6">
                <button
                  ...
                  className="w-full py-2 border border-ed-ink text-ed-ink text-[11px] font-semibold uppercase tracking-widest hover:bg-ed-ink hover:text-white transition-colors disabled:border-ed-border disabled:text-ed-muted disabled:cursor-wait"
                >
                  {gateLoading ? 'AI 평가 중…' : 'AI 게이트 평가 요청'}
                </button>
                {aiNotice && <p className="text-[11px] text-ed-accent mt-2 leading-snug">{aiNotice}</p>}
                {gateRec && (
                  <div className="mt-3 bg-white border border-ed-border p-4 space-y-2">
                    <p className="text-[11px] font-semibold text-ed-ink">AI 추천: {gateRec.result}</p>
                    <p className="text-[11px] text-ed-muted leading-snug">{gateRec.rationale}</p>
                    <div className="flex gap-2 pt-1">
                      <button onClick={applyGateRec} className="px-3 py-1.5 bg-ed-ink text-white text-[10px] font-semibold uppercase tracking-widest hover:bg-black transition-colors">적용 (수정 가능)</button>
                      <button onClick={() => setGateRec(null)} className="px-3 py-1.5 border border-ed-border text-ed-muted text-[10px] font-semibold uppercase tracking-widest hover:text-ed-ink transition-colors">무시</button>
                    </div>
                  </div>
                )}
              </div>
                ...
                {[
                  ...
                  <label key={item.key} className="flex items-start gap-3 p-2 hover:bg-[#eee] transition-colors cursor-pointer group rounded-sm">
                    <input 
                      ...
                      className="mt-[3px] accent-ed-ink w-3 h-3"
                    />
                    <span className={`text-[12px] leading-snug ${activeDraft.brandVoiceGate[item.key as keyof Draft['brandVoiceGate']] ? 'text-ed-ink font-semibold' : 'text-ed-muted group-hover:text-ed-ink'}`}>{item.label}</span>
                  </label>
                ))}
              </div>
              ...
              <div className={`p-6 border text-center bg-white ${
                ...
                <p className="text-[10px] text-ed-muted uppercase tracking-widest mb-2">Gate Status</p>
                <p className={`font-serif text-[18px] font-semibold tracking-tight ${
                ...
                <button 
                  ...
                  className={`mt-6 w-full py-3 text-[11px] font-semibold uppercase tracking-widest border transition-all ${
                    activeDraft.brandVoiceGate.result === 'Ready for review' 
                      ? 'bg-ed-ink text-white border-ed-ink hover:bg-black' 
                      : 'bg-transparent text-ed-muted border-ed-border cursor-not-allowed'
                  }`}
                >
                  리뷰 파이프라인으로 이동
                </button>
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-12 text-center bg-[#fafafa]">
             <div className="font-serif text-[2.5rem] text-ed-muted opacity-30 italic mb-4">No Draft Yet</div>
             <p className="text-[13px] text-ed-muted leading-relaxed max-w-[280px]">상단의 &quot;AI 초안 생성&quot;을 클릭하면 소스 데이터를 바탕으로 AI가 초안을 작성합니다. (키 미설정 시 템플릿 모드)</p>
          </div>
        )}
      </div>
    </div>
